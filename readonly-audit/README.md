...
中文：只读安全审计模式 —— 以只读工具集 + 审计 persona 进行代码安全审计的预设。纯预设版，不依赖任何自定义插件。

English: Read-only security audit mode — a preset for code security audits with a read-only tool set and an audit persona. Pure-preset edition: no custom plugin.
...
| 工具 / Tool | 模式下的行为 / Behavior in this mode |
|---|---|
...
## 与旧插件版的差异 / Differences from the old plugin edition
...
├── install-readonly-audit.sh     # macOS/Linux 安装码（自包含）
└── install-readonly-audit.ps1    # Windows 安装码（自包含，UTF-8 BOM）
```
